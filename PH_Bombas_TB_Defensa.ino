...
void on_message(const char* topic, byte* payload, unsigned int length) {
  ...
  // Obtener el nombre del método invocado
  
  String methodName = String((const char*)data["method"]);
  if(methodName="setPH"){
  Serial.print("Nombre metodo:");   
  Serial.println(methodName);
  String aux = String((const char*)data["params"]);
  ph=aux.toFloat();
  }
  
  //Capturo el atributo/telemetria que me mandaron
  ...
  String PPHH1 = String((const char*)data["ph"]);
  if(PPHH1!=""){
   ph=PPHH1.toFloat();
  Serial.println(ph);
  }
...
void compensarPH(){
  ...
  if(((deltaPH)>toleranciaPh)||(((-1)*deltaPH)>toleranciaPh)){ //Veo si esta fuera del rango de tolerancia
    ...
    float tiempo = deltaPH*5; // 5 segundo por cada unidad de diferencia de ph
    if(deltaPH>0){
